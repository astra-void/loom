...
function writeConsumerFixture(consumerRoot) {
	...
	writeFileSync(
		...
		[
			...
			"const previewMarkup = renderPreviewToString({",
			"\tentry: {",
			...
			"\t},",
			"});",
			'assert.match(previewMarkup, /packed-consumer-client/, "Expected @loom-dev/preview/client to prerender inline preview markup.");',
			"",
