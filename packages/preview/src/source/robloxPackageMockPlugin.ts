...
function isMockResolvedId(id: string) {
	...
	return (
		normalizedId === UNRESOLVED_MOCK_MODULE_ID ||
		normalizedId === RESOLVED_UNRESOLVED_MOCK_MODULE_ID
	);
}
...
async function shouldMockSpecifier(
	...
	try {
		...
		return (
			resolvedId == null ||
			isMockResolvedId(resolvedId) ||
			isUnsupportedResolvedId(resolvedId)
		);
	} catch {
		return true;
	}
...
function createValueAccessExpression(
		...
		(context) => {
			const visit: ts.Visitor = (currentNode) => {
				if (
					...
					currentNode.arguments.length === 1
				) {
					...
					if (
						...
						shouldMock(argument.text)
					) {
						if (
							ts.isIdentifier(currentNode.expression) &&
							currentNode.expression.text === "require"
						) {
							onRewrite();
	...
	return {
		...
		async transform(this: TransformResolveContext, code, id) {
			...
			if (
				...
				!/(?:import|export|require\s*\(|import\s*\()/.test(code)
			) {
				return undefined;
			}
			...
			const mockLocalName = createUniqueLocalName(
				code,
				DEFAULT_MOCK_IMPORT_BASENAME,
			);
			const moduleMockLocalName = createUniqueLocalName(
				code,
				MODULE_MOCK_IMPORT_BASENAME,
			);
			...
			for (const statement of sourceFile.statements) {
				if (
					...
					if (importClause.name) {
						...
						rewrittenStatements.push(
							createConstAlias(
								...
								ts.factory.createIdentifier(mockLocalName),
							),
						);
					}
					...
					for (const element of namedBindings.elements) {
						...
						rewrittenStatements.push(
							createConstAlias(
								element.name.text,
								createValueAccessExpression(mockLocalName, importedName),
							),
						);
					}
				...
				if (
					...
					for (const element of statement.exportClause.elements) {
						...
						const initializer = createValueAccessExpression(
							mockLocalName,
							importedName,
						);
						...
						rewrittenStatements.push(
							createExportedConst(exportedName, initializer),
						);
					}
				...
				const rewrittenStatement = rewriteExpressionLevelLoads(
					statement,
					moduleMockLocalName,
					shouldMock,
					() => {
			...
			return {
				code: printer.printFile(
					ts.factory.updateSourceFile(
						sourceFile,
						ts.factory.createNodeArray(rewrittenStatements),
					),
				),
				map: null,
			};
...
export const createRobloxPackageMockResolvePlugin =
	createUnresolvedPackageMockResolvePlugin;
export const createRobloxPackageMockTransformPlugin =
