...
function isSupportedComponentFile(filePath: string) {
	...
	return (
		SUPPORTED_COMPONENT_EXTENSIONS.has(path.extname(normalizedPath)) &&
...
function createProgram(
	...
	host.fileExists = (candidate) =>
		normalizeFilePath(candidate) === currentFilePath ||
	...
	for (const statement of sourceFile.statements) {
		...
		if (ts.isVariableStatement(statement) && hasExportModifier(statement)) {
			for (const declaration of statement.declarationList.declarations) {
				if (
					ts.isIdentifier(declaration.name) &&
					localDeclarations.has(declaration.name.text)
				) {
...
function isAnonymousDefaultExportExpression(
	expression: ts.Expression,
): boolean {
...
function includesUndefined(type: ts.Type): boolean {
	...
	return (
		type.isUnion() &&
		type.types.some((member): boolean => includesUndefined(member))
	);
}
...
function isReactNodeType(typeText: string) {
	return (
		...
		(typeText.includes("React.ReactElement") &&
			typeText.includes("React.ReactPortal"))
	);
}
...
function createLiteralMetadata(
	type: ts.Type,
	typeText: string,
): PreviewPropMetadata | undefined {
...
function serializePropertySymbol(
	...
	metadata.required =
		(propertySymbol.flags & ts.SymbolFlags.Optional) === 0 &&
		!includesUndefined(propertyType);
	return metadata;
}
...
function serializeType(
	...
	if (type.isUnion()) {
		...
		return {
			...
			required: true,
			type: typeText,
			unionTypes: filteredTypes.map((member) =>
				serializeType(member, checker, contextNode, seenTypes),
			),
		};
	}
	...
	if (checker.isArrayType(type) || checker.isTupleType(type)) {
		...
		return {
			...
			elementType: elementType
				? serializeType(elementType, checker, contextNode, seenTypes)
				: undefined,
			required: true,
			type: typeText,
		};
	...
	if ((type.flags & ts.TypeFlags.Object) !== 0) {
		...
		if (
			propertySymbols.length === 0 ||
			propertySymbols.length > MAX_SERIALIZED_OBJECT_PROPERTIES
		) {
			return {
				kind: "object",
				required: true,
		...
		for (const propertySymbol of propertySymbols) {
			...
			properties[propertySymbol.getName()] = serializePropertySymbol(
				propertySymbol,
				checker,
				contextNode,
				seenTypes,
...
function createComponentMetadata(
	...
	return createMetadataFromSignature(
		componentName,
		signature,
		declaration,
		checker,
...
function createMetadataFromSignature(
	...
	const propsDeclarationNode =
		propsParameter.valueDeclaration ?? declarationNode;
	const propsType = checker.getTypeOfSymbolAtLocation(
		propsParameter,
		propsDeclarationNode,
	);
	const props: Record<string, PreviewPropMetadata> = {};

	for (const propertySymbol of checker.getPropertiesOfType(propsType)) {
		props[propertySymbol.getName()] = serializePropertySymbol(
			propertySymbol,
			checker,
			propsDeclarationNode,
			new Set(),
...
function createAnonymousDefaultExportMetadata(
	sourceFile: ts.SourceFile,
	checker: ts.TypeChecker,
) {
	for (const statement of sourceFile.statements) {
		if (
			...
			const targetExpression = ts.isParenthesizedExpression(
...
function normalizeAnonymousDefaultExport(
	sourceFile: ts.SourceFile,
	code: string,
) {
	for (const statement of sourceFile.statements) {
		if (
			...
			const expressionText = code.slice(
		...
		if (
			...
			const statementText = code.slice(
				statement.getStart(sourceFile),
				statement.end,
			);
...
function injectComponentMetadata(
	...
	componentMetadata: Array<{
		metadata: PreviewComponentPropsMetadata;
		name: string;
	}>,
) {
...
export function createAutoMockPropsPlugin(
	...
	return {
		...
		transform(code, id) {
			...
			const defaultExportMetadata = createAnonymousDefaultExportMetadata(
				sourceFile,
				checker,
			);
			...
			const injectedMetadata = collectExportedComponentNames(
				sourceFile,
				localDeclarations,
			)
				.map((name) => {
			...
			const normalizedCode = defaultExportMetadata
				? normalizeAnonymousDefaultExport(sourceFile, code)
				: code;
			const transformedCode = injectComponentMetadata(
				normalizedCode,
				injectedMetadata,
			);
			if (!transformedCode) {
