...
const PreviewThemeContext =
	React.createContext<PreviewThemeContextValue | null>(null);
...
export function PreviewThemeProvider(props: { children: React.ReactNode }) {
	const [mode, setMode] = React.useState<PreviewThemeMode>(() =>
		readStoredThemeMode(),
	);
	const [systemTheme, setSystemTheme] = React.useState<PreviewResolvedTheme>(
		() => readSystemTheme(),
	);
...
export function usePreviewTheme() {
	...
	if (!context) {
		throw new Error(
			"usePreviewTheme must be used within PreviewThemeProvider.",
		);
	}
