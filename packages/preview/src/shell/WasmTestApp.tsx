...
export function WasmTestApp() {
	...
	const [layoutResult, setLayoutResult] = React.useState<
	...
	React.useEffect(() => {
		...
		const initialize = async () => {
			try {
				...
				const rawComputed = compute_layout(
					createMockTree(),
					VIEWPORT_WIDTH,
					VIEWPORT_HEIGHT,
				) as unknown;
				...
				if (!computed.CenterBox) {
					throw new Error(
						"Layout result does not include `CenterBox`. Check serialization shape from Wasm bridge.",
					);
				}
	...
	const renderedNodes = Object.entries(layoutResult).filter(
		([id]) => id !== "Root",
	);
			...
			>
				<section className="wasm-card">
					<div className="preview-empty preview-empty-centered">
						...
						<p>
							Fetching and validating the layout binary before computing the
							mock tree.
						</p>
					</div>
				</section>
			...
			>
				<section className="wasm-card">
					<div className="preview-empty preview-empty-centered">
						...
						<p>
							The layout engine could not be booted inside the preview shell.
						</p>
