...
function getComparableFilePaths(filePath: string) {
	return [
		...new Set([resolveFilePath(filePath), resolveRealFilePath(filePath)]),
	];
}
...
function isTraceableSourceFile(filePath: string) {
	...
	return [...TRACEABLE_SOURCE_EXTENSIONS].some((extension) =>
...
function combineSnapshots(
	...
	for (const target of targets) {
		...
		for (const [
			entryId,
			dependencyPaths,
		] of snapshot.entryDependencyPathsById.entries()) {
...
function collectImpactedEntryIds(
	snapshot: CombinedSnapshotState | undefined,
	filePaths: string[],
) {
	if (!snapshot) {
		return [];
	...
	for (const [
		...
		if (
			dependencyPaths.some((dependencyPath) =>
				getComparableFilePaths(dependencyPath).some((path) =>
					normalizedPaths.has(path),
				),
			)
		) {
...
function relativeToPackage(packageRoot: string, filePath: string) {
	const normalizedPackageRoot = resolveRealFilePath(packageRoot).replace(
		/\\/g,
		"/",
	);
...
function createReadyStatusDetails(
	...
	const warningDiagnostics = getWarningDiagnostics([
		...transformDiagnostics,
		...runtimeDiagnostics,
	]);
	const warningCodes = [
		...new Set(warningDiagnostics.map((diagnostic) => diagnostic.code)),
	...
	const degradedTargets = [
		...new Set(
			runtimeDiagnostics
				.filter(
					(diagnostic) =>
						diagnostic.code === "DEGRADED_HOST_RENDER" &&
...
function createPayloadHash(
	...
	const dependencyHashes = entryState.dependencyPaths
		.map((dependencyPath) => {
			const sourceText = fs.existsSync(dependencyPath)
...
function mergeTransformOutcome(
	...
	if (normalizedNext.kind === "mocked" || current.kind === "mocked") {
		return {
			fidelity:
				normalizedNext.fidelity === "degraded" ||
				current.fidelity === "degraded"
					? "degraded"
					: "preserved",
			kind: "mocked",
		};
	...
	if (
		normalizedNext.kind === "compatibility" ||
		current.kind === "compatibility"
	) {
		return {
			fidelity: "degraded",
			kind: "compatibility",
	...
	return {
		fidelity:
			normalizedNext.fidelity === "degraded" || current.fidelity === "degraded"
				? "degraded"
				: "preserved",
		kind: "ready",
	};
...
function computeTransformState(
	...
	for (const dependencyPath of entryState.dependencyPaths) {
		...
		for (const diagnostic of transformed.diagnostics) {
			const nextDiagnostic = toTransformDiagnostic(
				entryState,
				entryId,
				diagnostic,
			);
...
function mergeDiagnostics(
	...
	const diagnostics = [
		...entryState.discoveryDiagnostics,
		...transformDiagnostics,
		...runtimeDiagnostics,
	];
	const byPhase = {
...
function resolvePayloadStatus(
	...
	if (
		transform.outcome.kind === "blocked" ||
		transform.outcome.kind === "design-time"
	) {
		return {
	...
	const blockingRuntimeIssues = runtimeDiagnostics.filter(
		(diagnostic) =>
			diagnostic.phase === "runtime" && isBlockingDiagnostic(diagnostic),
	);
	...
	const blockingLayoutIssues = runtimeDiagnostics.filter(
		(diagnostic) =>
			diagnostic.phase === "layout" && isBlockingDiagnostic(diagnostic),
	);
	...
	return {
		status: "ready",
		statusDetails: createReadyStatusDetails(
			transform,
			transformDiagnostics,
			runtimeDiagnostics,
		),
...
function collectChangedEntryIds(
	...
	const previousEntriesById = new Map(
		previousWorkspaceIndex.entries.map((entry) => [entry.id, entry]),
	);
	...
	for (const entry of nextWorkspaceIndex.entries) {
		...
		if (
			!previousEntry ||
			JSON.stringify(previousEntry) !== JSON.stringify(entry)
		) {
...
class PreviewEngineImpl implements PreviewEngine {
	...
	public isTrackedSourceFile(filePath: string) {
		...
		return this.normalizedTargets.some((target) =>
			comparablePaths.some((candidatePath) =>
				isFilePathIncludedByTarget(target, candidatePath),
			),
		);
	}
	...
	public invalidateSourceFiles(filePaths: string[]) {
		...
		for (const target of affectedTargets) {
			this.targetSnapshots.set(
				createTargetKey(target),
				buildTargetSnapshot(this.options.projectName, target),
			);
		}
		...
		const registryChangedEntryIds = collectChangedEntryIds(
			...
			nextWorkspaceIndex,
			[...previousImpactedIds, ...nextImpactedIds],
		);
		...
		const update: PreviewEngineUpdate = {
			...
			requiresFullReload: false,
			workspaceChanged:
				registryChangedEntryIds.length > 0 || removedEntryIds.length > 0,
	...
	public replaceRuntimeIssues(issues: PreviewRuntimeIssue[]) {
		...
		const changedEntryIds = new Set<string>([
			...previousEntryIds,
			...nextRuntimeIssuesByEntryId.keys(),
		]);
