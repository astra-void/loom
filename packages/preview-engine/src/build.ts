...
function getBuildVersionFingerprint() {
	return {
		...
		previewRuntimeVersion: readPackageVersion(
			"../../preview-runtime/package.json",
		),
	...
	for (const context of targetContexts) {
		...
		for (const sourceFilePath of sourceFiles) {
			...
			const dependencyPaths =
				graphService.collectTransitiveDependencyPaths(sourceFilePath);
			...
			records.push({
				...
				relativePath: normalizeRelativePath(
					path.relative(context.sourceRoot, sourceFilePath),
				),
				sourceFilePath,
				sourceHash,
...
export async function buildPreviewArtifacts(
	...
	const cacheDir = resolveRealFilePath(
		options.cacheDir ?? path.join(workspaceRoot, ".loom-preview-cache"),
	);
	const concurrency =
		options.concurrency ??
	...
	const moduleRecords = artifactKinds.includes("module")
		? createModuleRecords(targetContexts, graphService)
		: [];
	...
	await runWithConcurrency(concurrency, moduleRecords, async (record) => {
		...
		if (!cachedRecord) {
			throw new Error(
				`Preview build cache did not materialize module metadata for ${record.sourceFilePath}.`,
			);
		}

		const diagnosticsSummary = createDiagnosticsSummary(
			cachedRecord.diagnostics,
		);
	...
	if (
		...
		writeJsonFile(
			path.join(getNamespaceDir(cacheDir, "manifests"), `${manifestKey}.json`),
			{
				artifactKinds,
				builtArtifactIds: sortBuiltArtifacts(builtArtifacts).map(
	...
	const blockingTransformDiagnostics = collectBlockingTransformDiagnostics(
		diagnostics,
	...
	if (options.outDir) {
		...
		const shouldMaterializeModuleArtifacts =
			blockingTransformDiagnostics.length === 0;

		for (const [relativeOutputPath, fileRecord] of [
			...materializedFiles.entries(),
			...
			const absoluteOutputPath = createMaterializedFilePath(
				options.outDir,
				relativeOutputPath,
			);
			...
			if (
				previousEntry?.cacheKey === fileRecord.cacheKey &&
		...
		for (const builtArtifact of builtArtifacts) {
			...
			builtArtifact.materializedPath = createMaterializedFilePath(
				options.outDir,
				manifestEntry[0],
			);
		}
