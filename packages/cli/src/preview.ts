...
export interface CliPreviewModule {
  ...
  loadPreviewConfig(options?: { configFile?: string; cwd?: string }): Promise<CliResolvedPreviewConfig>;
...
function createRuntime(runtimeOverrides: Partial<CliCommandRuntime>): ResolvedCliCommandRuntime {
  return {
    loadPreviewModuleFn: runtimeOverrides.loadPreviewModuleFn ?? loadPreviewModule,
...
export async function runSnapshotCommand(
  ...
  const effectiveConfig = applyResolvedConfigOverrides(resolvedConfig, options);
  const session = await previewModule.createPreviewHeadlessSession(effectiveConfig);

  try {
    const serializedSnapshot = `${JSON.stringify(session.getSnapshot(), null, 2)}\n`;
    if (options.outputPath) {
      await runtime.writeFileFn(path.resolve(options.outputPath), serializedSnapshot, "utf8");
      return;
    }
