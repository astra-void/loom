...
const DEFAULT_SOURCE_SANS_FAMILY =
	'"Source Sans 3", "Source Sans Pro", "Segoe UI", sans-serif';
...
function areElementSizesEqual(
	left: ElementSize | null,
	right: ElementSize | null,
) {
...
function useObservedElementSize(
	...
	React.useLayoutEffect(() => {
		...
		const update = (nextRect?: DOMRectReadOnly | ClientRect | null) => {
			...
			setSize((previous) =>
				areElementSizesEqual(previous, nextSize) ? previous : nextSize,
			);
		};
...
function measureScaledFontSize(
	options: TextScaleOptions & { size: ElementSize },
) {
	...
	measurement.style.fontWeight =
		options.fontWeight === undefined ? "400" : String(options.fontWeight);
	measurement.style.lineHeight = String(
		toNumericLineHeight(options.lineHeight),
	);
	...
	measurement.style.width = options.wrapped
		? `${options.size.width}px`
		: "max-content";
	measurement.textContent =
		options.text && options.text.length > 0 ? options.text : " ";
	document.body.appendChild(measurement);

	const fits = (fontSize: number) => {
		measurement.style.fontSize = `${fontSize}px`;
		return (
			measurement.scrollWidth <= options.size.width + 0.5 &&
			measurement.scrollHeight <= options.size.height + 0.5
		);
	};
