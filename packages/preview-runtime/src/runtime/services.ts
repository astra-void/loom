...
import { type RBXScriptConnection, type RBXScriptSignal, RunService } from "./RunService";
...
const USER_INPUT_TRACKER_KEY = Symbol.for("loom-dev.preview-runtime.userInputTracker");
...
export interface PreviewTweenService {
  ...
  Create(instance: unknown, tweenInfo: TweenInfo, goal: Record<string, unknown>): PreviewTween;
  GetFullName(): string;
  IsA(name: string): boolean;
}
...
function withRobloxFallback<T extends object>(target: T): T {
  return new Proxy(target, {
    ...
    has(currentTarget, property) {
      return Reflect.has(currentTarget, property) || typeof property === "string";
    },
  });
}
...
function createServiceBase(name: PreviewServiceName | "game") {
  return {
    ...
    IsA(typeName: string) {
      if (name === "game") {
        return typeName === "DataModel" || typeName === "ServiceProvider" || typeName === "Instance";
      }

      return typeName === name || typeName === "Service" || typeName === "Instance";
    },
  };
}
...
function getUserInputTracker() {
  ...
  if (!tracker.installed && typeof globalThis.addEventListener === "function") {
    ...
    globalThis.addEventListener("keydown", () => updateLastInputType("Keyboard"));
    globalThis.addEventListener("mousemove", () => updateLastInputType("MouseMovement"));
    ...
    globalThis.addEventListener("touchstart", () => updateLastInputType("Touch"));
...
function createWorkspaceService(): PreviewWorkspace {
  return withRobloxFallback({
    ...
    IsA(typeName: string) {
      return typeName === "Workspace" || typeName === "WorldRoot" || typeName === "Model" || typeName === "Instance";
    },
  });
}
