...
function normalizeLuaIndex(
	length: number,
	index: number | undefined,
	fallback: number,
) {
	const resolvedIndex =
		index == null || !Number.isFinite(index) ? fallback : Math.trunc(index);
...
function luaSubstring(value: string, start?: number, finish?: number) {
	...
	const normalizedStart = Math.max(
		1,
		Math.min(value.length + 1, normalizeLuaIndex(value.length, start, 1)),
	);
	const normalizedFinish = Math.max(
		0,
		Math.min(
			value.length,
			normalizeLuaIndex(value.length, finish, value.length),
		),
	);
...
function createCallableSize(readSize: () => number): CallableSize {
	...
	callable[Symbol.toPrimitive] = (hint) =>
		hint === "string" ? String(readSize()) : readSize();
	return callable;
}
...
function installCollectionSizeShim(prototype: object) {
	...
	Object.defineProperty(prototype, "size", {
		...
		get(this: object) {
			...
			if (!callableSize) {
				callableSize = createCallableSize(() =>
					Number(nativeGetSize.call(this)),
				);
...
function installStringPolyfills() {
	...
	definePrototypeMethod(
		String.prototype,
		"sub",
		function sub(this: string, start?: number, finish?: number) {
...
function installArrayPolyfills() {
	definePrototypeMethod(Array.prototype, "size", function size<
		T,
	>(this: Array<T>) {
		return this.length;
	});
	definePrototypeMethod(Array.prototype, "isEmpty", function isEmpty<
		T,
	>(this: Array<T>) {
...
export function installPreviewRuntimePolyfills(
	target: PreviewPolyfillTarget = globalThis as PreviewPolyfillTarget,
) {
