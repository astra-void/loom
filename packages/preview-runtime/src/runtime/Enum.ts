...
function resolveProxyMember(
	...
	if (path.length === 1) {
		...
		if (property === "FromName") {
			return (itemName: string) =>
				getItemProxy([categoryName, String(itemName)]);
		}

		if (property === "FromValue") {
			return (value: number) =>
				getItemProxy([categoryName, `Value${Number(value)}`]);
		}
	...
	if (property === "IsA") {
		return (name: string) =>
			name === "EnumItem" || name === categoryName || name === itemName;
	}
	...
	const proxy = new Proxy(Object.create(null) as EnumProxy, {
		...
		getOwnPropertyDescriptor(_target, property) {
			...
			return {
				configurable: true,
				enumerable:
					typeof property === "string" && reservedKeys.includes(property),
				value,
				writable: false,
			};
		},
