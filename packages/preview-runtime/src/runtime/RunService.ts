...
class PreviewRunServiceImpl implements PreviewRunService {
	...
	private readonly syncFrameLoop = () => {
		const hasListeners =
			this.renderSteppedSignal.size > 0 ||
			this.heartbeatSignal.size > 0 ||
			this.steppedSignal.size > 0;
	...
	private readonly renderSteppedSignal = new Signal<[deltaTime: number]>(
		this.syncFrameLoop,
	);
	private readonly heartbeatSignal = new Signal<[deltaTime: number]>(
		this.syncFrameLoop,
	);
	private readonly steppedSignal = new Signal<
		[time: number, deltaTime: number]
	>(this.syncFrameLoop);

	public readonly RenderStepped: RBXScriptSignal<[deltaTime: number]> =
		this.renderSteppedSignal;
	public readonly Heartbeat: RBXScriptSignal<[deltaTime: number]> =
		this.heartbeatSignal;
	public readonly Stepped: RBXScriptSignal<[time: number, deltaTime: number]> =
		this.steppedSignal;
