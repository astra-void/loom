	...
	React.useLayoutEffect(() => {
		...
		const update = (nextViewport?: ViewportSize | null) => {
			...
			setViewport((previous) =>
				areViewportsEqual(previous, next) ? previous : next,
			);
		};
		...
		if (typeof ResizeObserver !== "undefined") {
			const observer = new ResizeObserver((entries) => {
				const entry =
					entries.find(
						(candidate) => candidate.target === measurementElement,
					) ?? entries[0];
				update(
					createViewportSize(
			...
			() => {
				try {
					...
					publishPreviewRuntimeIssue(
						normalizePreviewRuntimeError(
							{
								code: isValidationError(nextError)
									? "LAYOUT_VALIDATION_ERROR"
									: "LAYOUT_WASM_COMPUTE_FAILED",
								kind: isValidationError(nextError)
...
function isPreviewLayoutNode(
	input: RobloxLayoutRegistrationInput | PreviewLayoutNode,
): input is PreviewLayoutNode {
	return (
		"layout" in input && "kind" in input && typeof input.nodeType === "string"
	);
}
...
export function useRobloxLayout(
	...
	const fallbackViewportWidth =
		context?.viewport.width ?? DEFAULT_VIEWPORT_WIDTH;
	const fallbackViewportHeight =
		context?.viewport.height ?? DEFAULT_VIEWPORT_HEIGHT;
