...
type DeepMockMember = ((...args: unknown[]) => unknown) &
...
function createSmartString(propName: string) {
	...
	const matchedHeuristic = SMART_STRING_HEURISTICS.find((heuristic) =>
		heuristic.matches(normalizedPropName),
	);
...
function shouldAutoMockOptionalProp(
	definition: PreviewPropMetadata,
	propName: string,
): boolean {
	...
	switch (definition.kind) {
		...
		case "string":
			return true;
		case "union":
			return (definition.unionTypes ?? []).some((option) =>
				shouldAutoMockOptionalProp(option, propName),
			);
		default:
			return false;
	}
...
function createObjectMock(definition: PreviewPropMetadata, propName: string) {
	...
	for (const [childName, childDefinition] of Object.entries(
		...
		if (
			!childDefinition.required &&
			!shouldAutoMockOptionalProp(childDefinition, childPropName)
		) {
			continue;
		}
...
function createMockValue(
	...
	if (
		shouldUseIndexHeuristic(propName) &&
...
export function buildAutoMockProps<Props extends Record<string, unknown>>(
	...
	const resolvedProps: Record<string, unknown> = explicitProps
		? { ...explicitProps }
		: {};
	...
	for (const [propName, definition] of Object.entries(metadata.props)) {
		...
		if (
			!definition.required &&
			!shouldAutoMockOptionalProp(definition, propName)
		) {
			continue;
		}
