...
const previewHostMetadataDocument =
	rawPreviewHostMetadata as PreviewHostMetadataDocument;
...
function createHostNodeTypeMap(records: PreviewHostMetadataRecord[]) {
	return Object.freeze(
		Object.fromEntries(
			records.map((record) => [record.jsxName, record.runtimeName]),
...
function validatePreviewHostMetadata(records: PreviewHostMetadataRecord[]) {
	...
	for (const record of records) {
		if (jsxNames.has(record.jsxName)) {
			throw new Error(
				`Duplicate preview host metadata jsxName: ${record.jsxName}`,
			);
		}

		if (runtimeNames.has(record.runtimeName)) {
			throw new Error(
				`Duplicate preview host metadata runtimeName: ${record.runtimeName}`,
			);
		}
...
export const previewHostMetadataByRuntimeName = new Map(
	previewHostMetadataRecords.map(
		(record) => [record.runtimeName, record] as const,
	),
);
...
export const layoutHostNodeType = createHostNodeTypeMap([
	...layoutHostMetadataRecords,
]);
export const fullSizeLayoutHostNames = Object.freeze(
	layoutHostMetadataRecords
		.filter((record) => record.fullSizeDefault)
...
export const previewHostDomTags = Object.freeze(
	Object.fromEntries(
		previewHostMetadataRecords.map((record) => [record.jsxName, record.domTag]),
	) as Record<string, string>,
);
export const supportedHostJsxNames = Object.freeze(
	previewHostMetadataRecords.map((record) => record.jsxName),
);
export const supportedHostRuntimeNames = Object.freeze(
	previewHostMetadataRecords.map((record) => record.runtimeName),
);
...
export function isPreviewHostTypeSupported(
	...
	const supportedSet =
		kind === "isa" ? supportedIsaNames : supportedTypeRewriteNames;
	return supportedSet.includes(typeName);
}

export function previewHostMatchesType(
	jsxName: string,
	typeName: string,
	kind: "isa" | "typeRewrite" = "isa",
) {
	...
	if (
		(kind === "isa" && !record.supportsIsa) ||
		(kind === "typeRewrite" && !record.supportsTypeRewrite)
	) {
		return false;
	}

	return (
		record.runtimeName === typeName ||
		record.abstractAncestors.includes(typeName)
	);
}
