...
describe("preview shell", () => {
	...
	it("shows blocked-by-transform guidance without rendering the canvas", async () => {
		...
		expect(
			await screen.findByText("This preview is blocked by transform mode."),
		).toBeTruthy();
		expect(
	...
	it("shows ambiguous guidance with concrete candidates", () => {
				...
				entryPayloads={{
					[ambiguousEntry.id]: createPayload(ambiguousEntry, [
						createDiagnostic({
							...
							summary:
								"Multiple component exports need explicit disambiguation: Alpha, Beta.",
						}),
					]),
				}}
		...
		expect(
			screen.getByText("Multiple exported components match this file."),
		).toBeTruthy();
		expect(
			screen.getByText(
	...
	it("shows no-component guidance without falling back to ambiguous messaging", () => {
		...
		expect(
			screen.getByText("This file is not directly previewable yet."),
		).toBeTruthy();
		expect(
			screen.getByText(
	...
	it("shows an empty-project state when there are no eligible preview entries", () => {
		renderPreviewApp(
			<PreviewApp
				entries={[]}
				loadEntry={() => Promise.reject(new Error("should not load"))}
				projectName="Empty"
			/>,
		);

		expect(
			screen.getByText("No previewable source files were found."),
		).toBeTruthy();
	});
			...
			<PreviewApp
				...
				loadEntry={() =>
					createLoadedEntry(staleEntry, {
						AnimatedSlot: () => (
							<button type="button">Recovered stale export</button>
						),
					})
				}
				projectName="@loom-dev/preview-smoke"
