...
describe("geometry fidelity", () => {
	it("observes derived AbsolutePosition changes synchronously during tweening", async () => {
		...
		const tweenInfo = new TweenInfo(1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut);
		const tweenService = game.GetService("TweenService") as any;
		const tween = tweenService.Create(ref.current, tweenInfo, { Position: UDim2.fromOffset(50, 50) });
