...
function createTempWorkspacePackage(files: Record<string, string>) {
	const workspaceRoot = fs.mkdtempSync(
		path.join(os.tmpdir(), "loom-preview-build-engine-"),
	);
...
function createTempWorkspacePackages(
	packages: Record<string, Record<string, string>>,
) {
	const workspaceRoot = fs.mkdtempSync(
		path.join(os.tmpdir(), "loom-preview-build-workspace-"),
	);
	temporaryRoots.push(workspaceRoot);
	fs.writeFileSync(
		path.join(workspaceRoot, "pnpm-workspace.yaml"),
...
describe("buildPreviewArtifacts", () => {
	it("reuses a shared workspace cache across output directories", async () => {
		const { cacheDir, packageRoot, sourceRoot, workspaceRoot } =
      ...
      `,
		...
		expect(
			second.builtArtifacts.every((artifact) => artifact.reusedFromCache),
		).toBe(true);
	...
	it("recomputes only dependent artifacts when transitive inputs change", async () => {
      ...
      `,
		...
		fs.writeFileSync(
			path.join(sourceRoot, "support", "labelText.ts"),
			'export const labelText = "changed";\n',
			"utf8",
		);
		...
		const artifactsById = new Map(
			rebuilt.builtArtifacts.map((artifact) => [artifact.id, artifact]),
		);

		expect(artifactsById.get("fixture:index.tsx")?.reusedFromCache).toBe(false);
		expect(
			artifactsById.get("fixture:support/labelText.ts")?.reusedFromCache,
		).toBe(false);
	...
	it("treats cache keys as sensitive to transform mode and runtime module", async () => {
      ...
      `,
		...
		expect(
			same.builtArtifacts.every((artifact) => artifact.reusedFromCache),
		).toBe(true);
		...
		expect(
			changedTransformMode.builtArtifacts.every(
				(artifact) => !artifact.reusedFromCache,
			),
		).toBe(true);
		...
		expect(
			changedRuntimeModule.builtArtifacts.every(
				(artifact) => !artifact.reusedFromCache,
			),
		).toBe(true);
	});
	...
	it("builds entry metadata and layout schema sidecars in metadata-only mode", async () => {
      ...
      `,
		...
		expect(first.builtArtifacts.map((artifact) => artifact.kind)).toEqual([
			"entry-metadata",
			"layout-schema",
		]);
		expect(first.writtenFiles).toEqual(
	...
	it("reuses most artifacts in a large synthetic workspace warm build", async () => {
		...
		const { packageRoot, sourceRoot, workspaceRoot } =
			createTempWorkspacePackage(files);
		...
		fs.writeFileSync(
			path.join(sourceRoot, "shared", "theme.ts"),
			'export const themeName = "updated";\n',
			"utf8",
		);
		...
		expect(invalidated.reusedArtifacts.length).toBeLessThan(
			invalidated.builtArtifacts.length,
		);
	});
	...
	it("rebuilds target-owned modules when a workspace package dependency changes", async () => {
		...
		fs.writeFileSync(
			path.join(sharedTarget.sourceRoot, "theme.ts"),
			'export const themeName = "updated";\n',
			"utf8",
		);
		...
		expect(
			rebuilt.builtArtifacts.some((artifact) =>
				artifact.sourceFilePath.includes("/shared/"),
			),
		).toBe(false);
	});
});
