...
import type { CliPreviewModule, CliResolvedPreviewConfig } from "../../packages/cli/src/preview";
...
function createResolvedConfig(overrides: Partial<CliResolvedPreviewConfig> = {}): CliResolvedPreviewConfig {
...
describe("loom cli", () => {
  ...
  it("prints resolved config JSON for explicit config and package-root fallback", async () => {
    ...
    await runCli(["config", "--cwd", "/workspace", "--config", "/workspace/loom.config.ts"], {
    ...
    expect(JSON.parse(explicitWriter.read())).not.toHaveProperty("targetDiscovery");
  ...
  it("emits snapshots to stdout by default and writes files when requested", async () => {
    ...
    await runCli(["snapshot", "--output", outputPath, "--transform-mode", "compatibility"], {
      loadPreviewModuleFn: async () => fileModule.previewModule,
      stdout: fileWriter.writer,
    });

    expect(fileWriter.read()).toBe("");
    expect(JSON.parse(fs.readFileSync(outputPath, "utf8"))).toEqual(fileModule.snapshot);
    expect(fileModule.previewModule.createPreviewHeadlessSession).toHaveBeenCalledWith(
      expect.objectContaining({
        transformMode: "compatibility",
      }),
    );
  ...
  it("validates port, option values, and transform modes", async () => {
    ...
    await expect(runCli(["preview", "--transform-mode", "mocked"])).rejects.toMatchObject({
